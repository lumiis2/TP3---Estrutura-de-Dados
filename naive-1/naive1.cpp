...
#include <cstdio>    // Para printf, fopen, etc.
...
// struct that contains the distance information between the origin location and
...
// Função para imprimir o mapa ilustrativo usando gnuplot
void printmap(const knn_vector& kvet, int kmax, const vector<addr_t>& rvet, int nrec, double tx, double ty) {
    FILE* out;

    // Script do gnuplot. Para obter o mapa real, é necessário executar:
    // gnuplot out.gp
    out = fopen("out.gp", "wt");
    ...
    // Todas as localizações de recarga
    ...
    // As localizações de recarga mais próximas
...
int main(int argc, char** argv) {
    cout << "rodando essa porra" << endl;
    // Contar o número de locais de recarga em rechargevet
    int numrecharge = 0;
    while (rechargevet[numrecharge].id_logrado != -1) numrecharge++;

    // Ler as coordenadas do ponto atual que precisa de recarga
    double tx = atof(argv[1]);
    double ty = atof(argv[2]);

    // Criar o vetor de distâncias e preenchê-lo
    knn_vector kvet(numrecharge);
    ...
    // Imprimir as 10 localizações de recarga mais próximas
    int kmax = 10;
    for (int i = 0; i < kmax; i++) {
